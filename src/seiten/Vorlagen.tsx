...
export default function Vorlagen() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* ── Reiter ── */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: '#f5f3ef', borderRadius: 10, padding: 4 }}>
        {([
          { key: 'positionen',    label: '📋 Positionsvorlagen' },
          { key: 'abschlusstexte', label: '📝 Abschlusstexte' },
        ] as const).map(r => (
          <button key={r.key} onClick={() => setReiter(r.key)}
            style={{ flex: 1, padding: '8px 16px', border: 'none', borderRadius: 8, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: 13, fontWeight: reiter === r.key ? 700 : 400, background: reiter === r.key ? 'white' : 'transparent', color: reiter === r.key ? '#1a1a1a' : '#888', boxShadow: reiter === r.key ? '0 1px 4px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.15s' }}>
            {r.label}
          </button>
        ))}
      </div>
      ...
      {reiter === 'positionen' && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <button style={{ background: '#1a1a1a', color: 'white', border: 'none', borderRadius: 8, padding: '9px 18px', fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
            ...
            {vorlagen.length === 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                <div style={{ textAlign: 'center', color: '#888' }}>
                  ...
                  <div style={{ fontSize: 13, marginBottom: 20, color: '#aaa' }}>Speichere häufig verwendete Positionen,<br />um sie schnell einzufügen.</div>
                  <button style={{ background: '#c8a96e', color: '#0a0a0a', border: 'none', borderRadius: 8, padding: '10px 24px', fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
                ...
                <tbody>
                  {vorlagen.map((v: any) => (
                    <tr key={v.id} style={{ borderBottom: '1px solid #f0ede8' }}
                      ...
                      <td style={{ padding: '10px 14px', fontFamily: 'Syne, sans-serif', fontSize: 12, fontWeight: 700, color: '#1a1a1a', maxWidth: 160 }}>{v.name}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12, color: '#555', maxWidth: 260 }}>
                        <span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.beschreibung || <span style={{ color: '#ccc' }}>—</span>}</span>
                      </td>
                      <td style={{ padding: '10px 14px', fontSize: 12, color: '#888', textAlign: 'right' }}>{parseFloat(v.menge).toLocaleString('de-AT')}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12, color: '#888' }}>{v.einheit}</td>
            ...
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 100, overflowY: 'auto', padding: '40px 20px' }}>
              <div style={{ background: 'white', borderRadius: 14, width: 520, margin: 'auto', boxShadow: '0 24px 60px rgba(0,0,0,0.3)' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e0d8', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, flex: 1 }}>{bearbeitenId ? '✏️ Vorlage bearbeiten' : '📋 Neue Vorlage'}</div>
                  <button onClick={() => { setFormOffen(false); formLeeren() }} style={{ background: 'transparent', border: 'none', fontSize: 20, cursor: 'pointer', color: '#888' }}>✕</button>
                </div>
                <div style={{ padding: 24 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
                    ...
                    <div>
                      ...
                      <select style={{ ...inputStyle, background: 'white' }} value={einheit} onChange={e => setEinheit(e.target.value)}>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    <button style={{ flex: 1, padding: 13, background: '#1a1a1a', color: 'white', border: 'none', borderRadius: 9, fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 700, cursor: 'pointer' }} onClick={speichern} disabled={laden}>
                      {laden ? '⏳ Wird gespeichert...' : bearbeitenId ? '✅ Änderungen speichern' : '✅ Vorlage speichern'}
                    </button>
                    <button style={{ padding: 13, background: '#f0ede8', color: '#888', border: 'none', borderRadius: 9, cursor: 'pointer' }} onClick={() => { setFormOffen(false); formLeeren() }}>Abbrechen</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, overflowY: 'auto' }}>
            {abschlusstexte.length === 0 ? (
              <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e0d8', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
                <div style={{ textAlign: 'center', color: '#888' }}>
                  <div style={{ fontSize: 48, marginBottom: 16 }}>📝</div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 16, fontWeight: 700, marginBottom: 8, color: '#1a1a1a' }}>Noch keine Abschlusstexte</div>
                  <div style={{ fontSize: 13, marginBottom: 20, color: '#aaa' }}>Erstelle Vorlagen für Grußformeln und Abschlusstexte.</div>
                  <button style={{ background: '#c8a96e', color: '#0a0a0a', border: 'none', borderRadius: 8, padding: '10px 24px', fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
              ...
              abschlusstexte.map((t: any) => (
                <div key={t.id} style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e0d8', padding: '16px 18px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, color: '#1a1a1a', marginBottom: 8 }}>{t.name}</div>
                    <div style={{ fontSize: 12, color: '#555', lineHeight: 1.6, whiteSpace: 'pre-wrap', background: '#faf8f5', borderRadius: 6, padding: '10px 12px', border: '1px solid #f0ede8' }}>{t.text}</div>
                  </div>
          ...
          {atFormOffen && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 100, overflowY: 'auto', padding: '40px 20px' }}>
              <div style={{ background: 'white', borderRadius: 14, width: 580, margin: 'auto', boxShadow: '0 24px 60px rgba(0,0,0,0.3)' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e0d8', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, flex: 1 }}>{atBearbeitenId ? '✏️ Abschlusstext bearbeiten' : '📝 Neuer Abschlusstext'}</div>
                  <button onClick={() => { setAtFormOffen(false); atFormLeeren() }} style={{ background: 'transparent', border: 'none', fontSize: 20, cursor: 'pointer', color: '#888' }}>✕</button>
                </div>
                <div style={{ padding: 24 }}>
                  <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#aaa', marginTop: 4 }}>Neue Zeile = Zeilenumbruch im PDF. Leerzeile = Absatz.</div>
                  </div>
                  <div style={{ display: 'flex', gap: 10 }}>
                    <button style={{ flex: 1, padding: 13, background: '#1a1a1a', color: 'white', border: 'none', borderRadius: 9, fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 700, cursor: 'pointer' }} onClick={atSpeichern} disabled={atLaden}>
                      {atLaden ? '⏳ Wird gespeichert...' : atBearbeitenId ? '✅ Änderungen speichern' : '✅ Abschlusstext speichern'}
                    </button>
                    <button style={{ padding: 13, background: '#f0ede8', color: '#888', border: 'none', borderRadius: 9, cursor: 'pointer' }} onClick={() => { setAtFormOffen(false); atFormLeeren() }}>Abbrechen</button>
                  </div>
                </div>
              </div>
            </div>
