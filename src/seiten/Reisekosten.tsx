...
export default function Reisekosten() {
  ...
  const guvUebertragen = async (e: Reisekost) => {
    try {
      ...
      if (info === 'nichts_zu_uebertragen') {
        zeigeToast('KM-Geld ist im KM-Buch, kein Taggeld vorhanden – nichts zu übertragen', false)
      } else {
  ...
  const KmChip = ({ k, gewählt, onWähle, onLöse, farbe }: {
    ...
    <button onClick={gewählt ? onLöse : onWähle} style={{
      padding: '6px 12px', borderRadius: 9, border: `1.5px solid ${gewählt ? farbe : 'var(--bf-border)'}`,
      cursor: 'pointer', fontSize: 11, fontWeight: 700, transition: 'all 0.15s',
      background: gewählt ? farbe : 'var(--bf-card)',
      color: gewählt ? 'white' : 'var(--bf-text)',
      boxShadow: gewählt ? `0 3px 10px ${farbe}44` : 'var(--bf-shadow)',
  ...
  return (
    <div style={{ padding: isMobile ? '16px' : '28px', fontFamily: 'DM Sans, sans-serif', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={s.label} style={{ ...card, borderTop: `3px solid ${s.color}` }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: isMobile ? 18 : 22, fontWeight: 800, color: 'var(--bf-text)' }}>{s.value}</div>
        ...
        return (
          <div style={{ ...card, marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
              <div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 15, fontWeight: 800, color: 'var(--bf-text)' }}>Reisekosten – {filterJahr}</div>
              ...
              <div style={{ display: 'flex', background: 'var(--bf-soft)', borderRadius: 10, padding: 3, gap: 2 }}>
                {[{ k: 'jahr' as const, l: 'Jahresansicht' }, { k: 'monat' as const, l: 'Monatsdetail' }].map(t => (
                  <button key={t.k} onClick={() => setDiagrammAnsicht(t.k)} style={{
                    ...
                    boxShadow: diagrammAnsicht === t.k ? 'var(--bf-shadow)' : 'none', transition: 'all 0.2s',
                  }}>{t.l}</button>
                ))}
              </div>
            </div>
              ...
              <>
                <div style={{ display: 'flex', gap: 0, alignItems: 'flex-end' }}>
                  ...
                  <div style={{ flex: 1, position: 'relative' }}>
                    {[0, 0.25, 0.5, 0.75, 1].map((p, i) => (
                      <div key={i} style={{ position: 'absolute', left: 0, right: 0, bottom: 22 + Math.round(p * CHART_H), height: 1, background: i === 0 ? 'var(--bf-border)' : 'var(--bf-divider)', zIndex: 0 }} />
                    ))}
                      ...
                      {monatsDaten.map((m, i) => {
                        ...
                        return (
                          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer', position: 'relative', zIndex: 1 }}
                            ...
                            <div style={{ fontSize: isMobile ? 8 : 10, marginTop: 5, color: istAktMonat ? BLAU : m.hatDaten ? 'var(--bf-text-soft)' : 'var(--bf-text-muted)', fontWeight: istAktMonat ? 800 : 600 }}>{m.label}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, flexWrap: 'wrap', gap: 8 }}>
                  <div style={{ display: 'flex', gap: 18 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                      ...
                      <span style={{ color: 'var(--bf-text-soft)', fontWeight: 600 }}>absetzbar gesamt</span>
                    </div>
                  </div>
                ...
                {monatsDaten.some(m => m.hatDaten) && (
                  <div style={{ marginTop: 14, padding: '12px 14px', background: 'var(--bf-soft)', borderRadius: 10 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--bf-text-muted)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Absetzbar pro Monat</div>
                    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                      {monatsDaten.filter(m => m.hatDaten).map((m, i) => (
                        <div key={i} style={{ padding: '4px 9px', borderRadius: 7, fontSize: 10, fontWeight: 700, background: `${GOLD}18`, color: GOLD, border: `1px solid ${GOLD}44` }}>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 18 }}>
                  {monatsDaten.map((m, i) => (
                    <button key={i} onClick={() => setDiagrammMonat(m.monat)} style={{
                      ...
                      background: diagrammMonat === m.monat ? 'var(--bf-text)' : m.hatDaten ? 'var(--bf-soft)' : 'var(--bf-card)',
                    ...
                    <>
                      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
                        <div style={{ background: `${BLAU}10`, borderRadius: 12, padding: '12px 16px', border: `1px solid ${BLAU}22` }}>
                          ...
                          <div style={{ fontSize: 10, color: 'var(--bf-text-muted)', marginTop: 3 }}>{mDat.anzahl} Reisen</div>
                        </div>
                        <div style={{ background: `${GRUEN}10`, borderRadius: 12, padding: '12px 16px', border: `1px solid ${GRUEN}22` }}>
                          ...
                          <div style={{ fontSize: 10, color: 'var(--bf-text-muted)', marginTop: 3 }}>Diäten</div>
                        </div>
                        <div style={{ background: `${GOLD}12`, borderRadius: 12, padding: '12px 16px', border: `1px solid ${GOLD}33`, gridColumn: isMobile ? '1 / -1' : 'auto' }}>
                          <div style={{ fontSize: 10, color: GOLD, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6 }}>Absetzbar</div>
                          <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, color: GOLD }}>€ {fmt(mDat.gesamt)}</div>
                          <div style={{ fontSize: 10, color: 'var(--bf-text-muted)', marginTop: 3 }}>gesamt</div>
                        </div>
                      </div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--bf-text-muted)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12 }}>Reisen im {mDat.labelLang}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {monatEintraege.map(e => (
                          <div key={e.id}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                              <div style={{ flex: 1, overflow: 'hidden' }}>
                                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--bf-text)' }}>{e.zielort}</span>
                                <span style={{ fontSize: 11, color: 'var(--bf-text-muted)', marginLeft: 8 }}>{e.zweck}</span>
                              </div>
                              <span style={{ fontSize: 12, color: GOLD, fontWeight: 700, flexShrink: 0 }}>€ {fmt(Number(e.gesamt))}</span>
                            </div>
                            <div style={{ height: 8, background: 'var(--bf-soft)', borderRadius: 4, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        <button onClick={() => oeffneForm()} style={{ ...btnPrimary }}>+ Neue Dienstreise</button>
      ...
      {monate.length > 0 && (
        <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
          <button onClick={() => setFilterMonat('Alle')} style={{ padding: '5px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, background: filterMonat === 'Alle' ? 'var(--bf-text)' : 'var(--bf-soft)', color: filterMonat === 'Alle' ? 'var(--bf-card)' : 'var(--bf-text-muted)' }}>
            Alle ({eintraege.length})
          </button>
          {monate.map(m => {
            const count = eintraege.filter(e => e.datum?.startsWith(m)).length
            return (
              <button key={m} onClick={() => setFilterMonat(m)} style={{ padding: '5px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, background: filterMonat === m ? 'var(--bf-text)' : 'var(--bf-soft)', color: filterMonat === m ? 'var(--bf-card)' : 'var(--bf-text-muted)' }}>
      ...
      {laden ? (
        ...
        <div style={{ ...card, textAlign: 'center', padding: '50px 24px' }}>
          <div style={{ fontSize: 15, color: 'var(--bf-text-muted)', marginBottom: 8 }}>Noch keine Dienstreisen für {filterJahr}</div>
          <div style={{ fontSize: 12, color: 'var(--bf-text-muted)' }}>Klicke „+ Neue Dienstreise" für den ersten Eintrag</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {gefiltert.map(e => (
            <div key={e.id} style={{ ...card, cursor: 'pointer' }} onClick={() => setDetailEintrag(e)}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <div style={{ textAlign: 'center', minWidth: 46, background: 'var(--bf-soft)', borderRadius: 10, padding: '6px 4px' }}>
                  <div style={{ fontSize: 9, color: 'var(--bf-text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>
                    {e.datum ? new Date(e.datum).toLocaleDateString('de-AT', { month: 'short' }) : ''}
                  </div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, color: 'var(--bf-text)', lineHeight: 1 }}>
                    {e.datum ? new Date(e.datum).getDate() : ''}
                  </div>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--bf-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.zielort}</div>
                  <div style={{ fontSize: 12, color: 'var(--bf-text-muted)', marginTop: 2 }}>{e.zweck}</div>
                  <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, background: `${BLAU}12`, color: BLAU, padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>KM € {fmt(Number(e.km_geld))}</span>
                    {Number(e.taggeld) > 0 && <span style={{ fontSize: 11, background: `${GRUEN}12`, color: GRUEN, padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>Taggeld € {fmt(Number(e.taggeld))}</span>}
                    <span style={{ fontSize: 11, background: `${GOLD}18`, color: GOLD, padding: '2px 8px', borderRadius: 6, fontWeight: 700, border: `1px solid ${GOLD}44` }}>€ {fmt(Number(e.gesamt))}</span>
                    {e.uhrzeit_start && e.uhrzeit_ende && <span style={{ fontSize: 11, color: 'var(--bf-text-muted)', padding: '2px 4px' }}>{e.uhrzeit_start} – {e.uhrzeit_ende}</span>}
                    {e.km_fahrt_hin_id && <span style={{ fontSize: 11, background: `${GRUEN}12`, color: GRUEN, padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>Hin</span>}
                    {e.km_fahrt_rueck_id && <span style={{ fontSize: 11, background: `${BLAU}12`, color: BLAU, padding: '2px 8px', borderRadius: 6, fontWeight: 600 }}>Rück</span>}
                    {e.in_guv
                      ? <StatusChip status="Bezahlt" label="G&V" />
                      : <StatusChip status="Offen" label="offen" />
                    }
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 4, flexShrink: 0 }} onClick={ev => ev.stopPropagation()}>
                  {!e.in_guv && (
                    <button
                      ...
                      style={{ ...btnSecondary, padding: '6px 10px', fontSize: 11, fontWeight: 700, color: GOLD, borderColor: `${GOLD}66`, whiteSpace: 'nowrap' }}
                      title="In G&V übertragen"
                    >→ G&V</button>
                  )}
                  <button onClick={() => oeffneForm(e)} style={{ ...btnSecondary, padding: '6px 10px', fontSize: 11 }}>Bearbeiten</button>
                  <button onClick={() => setConfirmModal({ text: `Eintrag für „${e.zielort}" löschen?`, onJa: () => loeschen(e.id) })} style={{ ...btnSecondary, padding: '6px 10px', fontSize: 11, color: '#ef4444', borderColor: 'rgba(239,68,68,0.35)' }}>Löschen</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {formOffen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,10,20,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--bf-card)', borderRadius: 20, padding: 28, maxWidth: 580, width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 17, fontWeight: 800, color: 'var(--bf-text)', marginBottom: 20 }}>
              {editEintrag ? 'Dienstreise bearbeiten' : 'Neue Dienstreise'}
            </div>
            ...
            {kmTage.length > 0 && (
              <div style={{ marginBottom: 16, padding: '14px 16px', background: 'var(--bf-soft)', borderRadius: 14, border: `1px solid ${BLAU}22` }}>
                ...
                {(kmHinGewählt || kmRueckGewählt) && (
                  <div style={{ marginTop: 12, padding: '8px 10px', background: 'var(--bf-card)', borderRadius: 9, fontSize: 11, color: 'var(--bf-text-soft)', border: `1px solid var(--bf-border)` }}>
                    ...
                    {kmHinGewählt && kmRueckGewählt && (
                      <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px solid var(--bf-divider)', color: GOLD, fontWeight: 700 }}>
            ...
            {form.uhrzeit_start && form.uhrzeit_ende && vorschau.stunden > 0 && (
              <div style={{ fontSize: 11, color: 'var(--bf-text-muted)', marginTop: -10, marginBottom: 12, paddingLeft: 4 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {Number(form.km_einfach) > 0 && (
                <div style={{ fontSize: 11, color: 'var(--bf-text-muted)', marginTop: 4, paddingLeft: 4 }}>
            ...
            {/* Essen bekommen */}
            <div style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: 'var(--bf-soft)', borderRadius: 10 }}>
              ...
              <label htmlFor="essen" style={{ fontSize: 13, fontWeight: 600, color: 'var(--bf-text)', cursor: 'pointer' }}>
                Mittagessen vom Kunden bezahlt <span style={{ color: 'var(--bf-text-muted)', fontWeight: 400 }}>(−€ 15,00 vom Taggeld)</span>
              </label>
            </div>

            {/* Live-Vorschau */}
            {Number(form.km_einfach) > 0 && (
              <div style={{ marginBottom: 20, padding: '16px', background: `${GOLD}12`, borderRadius: 14, border: `1px solid ${GOLD}44` }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--bf-text-muted)' }}>{Number(form.km_einfach) * 2} km × € 0,50</div>
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--bf-text-muted)' }}>
                      {Number(form.km_einfach) < 25 ? 'unter 25 km' : vorschau.stunden <= 3 ? 'unter 3 Std.' : `${Math.ceil(vorschau.stunden)} Std. × € 2,50`}
                    </div>
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 11, color: GOLD, fontWeight: 700, marginBottom: 4 }}>Gesamt</div>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 20, fontWeight: 800, color: GOLD }}>€ {fmt(vorschau.gesamt)}</div>
                    <div style={{ fontSize: 10, color: 'var(--bf-text-muted)' }}>absetzbar</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => { setFormOffen(false); setEditEintrag(null) }} style={{ ...btnSecondary }}>Abbrechen</button>
              <button onClick={speichern} style={{ ...btnPrimary }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,10,20,0.65)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--bf-card)', borderRadius: 20, maxWidth: 520, width: '100%', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ background: `linear-gradient(135deg, ${GOLD}, #e8c98e)`, padding: '20px 24px' }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, color: '#0a0a0a' }}>{detailEintrag.zielort}</div>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '60vh', overflowY: 'auto' }}>
              {[
                { label: 'Datum', value: detailEintrag.datum ? new Date(detailEintrag.datum).toLocaleDateString('de-AT', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : '—' },
                { label: 'Reisezeit', value: detailEintrag.uhrzeit_start && detailEintrag.uhrzeit_ende ? `${detailEintrag.uhrzeit_start} – ${detailEintrag.uhrzeit_ende} (${Number(detailEintrag.stunden_dauer || 0).toFixed(1)} Std.)` : '—' },
                { label: 'Strecke', value: `${Number(detailEintrag.km_einfach).toFixed(1)} km einfach · ${(Number(detailEintrag.km_einfach) * 2).toFixed(1)} km gesamt` },
                { label: 'Mahlzeit gestellt', value: detailEintrag.essen_bekommen ? 'Ja – Taggeld um € 15,00 reduziert' : 'Nein' },
              ].map((r, i) => (
                <div key={i}>
              ...
              {/* Abrechnungs-Box */}
              <div style={{ background: `${GOLD}12`, borderRadius: 14, padding: '14px 16px', border: `1px solid ${GOLD}44` }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, textAlign: 'center' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, color: GOLD }}>€ {fmt(Number(detailEintrag.gesamt))}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '14px 24px', borderTop: '1px solid var(--bf-divider)' }}>
              <button onClick={() => setDetailEintrag(null)} style={{ ...btnSecondary }}>Schließen</button>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { oeffneForm(detailEintrag); setDetailEintrag(null) }} style={{ ...btnSecondary }}>Bearbeiten</button>
                <button onClick={() => { setConfirmModal({ text: `Eintrag für „${detailEintrag.zielort}" löschen?`, onJa: () => { loeschen(detailEintrag.id); setDetailEintrag(null) } }) }} style={{ ...btnSecondary, color: '#ef4444', borderColor: 'rgba(239,68,68,0.35)' }}>Löschen</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {confirmModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,10,20,0.6)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--bf-card)', borderRadius: 16, padding: '28px 24px', maxWidth: 400, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', textAlign: 'center' }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--bf-text)', marginBottom: 8, whiteSpace: 'pre-line' }}>{confirmModal.text}</div>
            {!confirmModal.text.includes('G&V') && (
              <div style={{ fontSize: 12, color: 'var(--bf-text-muted)', marginBottom: 24 }}>Diese Aktion kann nicht rückgängig gemacht werden.</div>
            )}
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 16 }}>
              <button onClick={() => setConfirmModal(null)} style={{ ...btnSecondary }}>Abbrechen</button>
              <button onClick={() => { confirmModal.onJa(); setConfirmModal(null) }}
                style={confirmModal.text.includes('G&V')
                  ? { ...btnPrimary }
                  : { ...btnSecondary, color: '#ef4444', borderColor: 'rgba(239,68,68,0.35)', fontWeight: 700 }}>
